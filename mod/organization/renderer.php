 ...
 */
class mod_organization_renderer extends plugin_renderer_base {
    ...
    /// Header setup
        ...
        if (has_capability('mod/organization:manage', $context)) {
         //   $output .= $this->output->heading_with_help($activityname, 'overview', MOD_ORGANIZATION_LANG);

            if (!empty($currenttab)) {
                ob_start();
                include($CFG->dirroot.'/mod/organization/tabs.php');
     ...
     *
     */
	public function show_intro($organization,$cm){
		$ret = "";
		if (trim(strip_tags($organization->intro))) {
			$ret .= $this->output->box_start('mod_introbox');
			$ret .= format_module_intro('organization', $organization, $cm->id);
			$ret .= $this->output->box_end();
		}
		return $ret;
	}
...
class mod_organization_report_renderer extends plugin_renderer_base {


	public function render_reportmenu($moduleinstance,$cm) {
		
		$basic = new single_button(
			new moodle_url(MOD_ORGANIZATION_URL . '/reports.php',array('report'=>'basic','id'=>$cm->id,'n'=>$moduleinstance->id)), 
			get_string('basicreport',MOD_ORGANIZATION_LANG), 'get');

		$ret = html_writer::div($this->render($basic) .'<br />'  ,MOD_ORGANIZATION_CLASS  . '_listbuttons');
	...
	public function render_delete_allattempts($cm){
		$deleteallbutton = new single_button(
				new moodle_url(MOD_ORGANIZATION_URL . '/manageattempts.php',array('id'=>$cm->id,'action'=>'confirmdeleteall')), 
				get_string('deleteallattempts',MOD_ORGANIZATION_LANG), 'get');
		$ret =  html_writer::div( $this->render($deleteallbutton) ,MOD_ORGANIZATION_CLASS  . '_actionbuttons');
		return $ret;
	}

	public function render_reporttitle_html($course,$username) {
		$ret = $this->output->heading(format_string($course->fullname),2);
		$ret .= $this->output->heading(get_string('reporttitle',MOD_ORGANIZATION_LANG,$username),3);
		return $ret;
	}

	public function render_empty_section_html($sectiontitle) {
		global $CFG;
		return $this->output->heading(get_string('nodataavailable',MOD_ORGANIZATION_LANG),3);
	}
	...
	public function render_exportbuttons_html($cm,$formdata,$showreport){
		...
		$pdf = new single_button(
			new moodle_url(MOD_ORGANIZATION_URL . '/reports.php',$formdata),
			get_string('exportpdf',MOD_ORGANIZATION_LANG), 'get');
		*/
		$formdata['format']='csv';
		$excel = new single_button(
			new moodle_url(MOD_ORGANIZATION_URL . '/reports.php',$formdata), 
			get_string('exportexcel',MOD_ORGANIZATION_LANG), 'get');

		return html_writer::div( $this->render($excel),MOD_ORGANIZATION_CLASS  . '_actionbuttons');
	}
	...
	public function render_section_html($sectiontitle, $report, $head, $rows, $fields) {
		...
		$tableattributes = array('class'=>'generaltable '. MOD_ORGANIZATION_CLASS .'_table');
		$headrow_attributes = array('class'=>MOD_ORGANIZATION_CLASS . '_headrow');
		...
		foreach($rows as $row){
			...
			foreach($fields as $field){
				...
				$cell->attributes= array('class'=>MOD_ORGANIZATION_CLASS . '_cell_' . $report . '_' . $field);
				$htr->cells[] = $cell;
			}
	...
	function show_reports_footer($moduleinstance,$cm,$formdata,$showreport){
		...
		$link = new moodle_url(MOD_ORGANIZATION_URL . '/reports.php',array('report'=>'menu','id'=>$cm->id,'n'=>$moduleinstance->id));
		$ret =  html_writer::link($link, get_string('returntoreports',MOD_ORGANIZATION_LANG));
