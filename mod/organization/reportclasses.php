 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class mod_organization_base_report {
	...
	public function fetch_head(){
		...
		foreach($this->fields as $field){
			$head[]=get_string($field,MOD_ORGANIZATION_LANG);
		}
		return $head;
	}
...
*
*/
class mod_organization_basic_report extends  mod_organization_base_report {
	...
	public function fetch_formatted_heading(){
		...
		//$ec = $this->fetch_cache(MOD_ORGANIZATION_TABLE,$record->englishcentralid);
		return get_string('basicheading',MOD_ORGANIZATION_LANG);
	...
	public function process_raw_data($formdata,$paging=false){
		...
		$emptydata = array();
		$alldata = $DB->get_records(MOD_ORGANIZATION_TABLE,array());
		if($alldata){
