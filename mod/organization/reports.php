...
if ($id) {
    $cm         = get_coursemodule_from_id(MOD_ORGANIZATION_MODNAME, $id, 0, false, MUST_EXIST);
    $course     = $DB->get_record('course', array('id' => $cm->course), '*', MUST_EXIST);
    $moduleinstance  = $DB->get_record(MOD_ORGANIZATION_TABLE, array('id' => $cm->instance), '*', MUST_EXIST);
} elseif ($n) {
    $moduleinstance  = $DB->get_record(MOD_ORGANIZATION_TABLE, array('id' => $n), '*', MUST_EXIST);
    $course     = $DB->get_record('course', array('id' => $moduleinstance->course), '*', MUST_EXIST);
    $cm         = get_coursemodule_from_instance(MOD_ORGANIZATION_TABLE, $moduleinstance->id, $course->id, false, MUST_EXIST);
} else {
    error('You must specify a course_module ID or an instance ID');
}

if($showreport=='menu'){
	$PAGE->set_url(MOD_ORGANIZATION_URL . '/reports.php', array('id' => $cm->id));
}else{
	$PAGE->set_url(MOD_ORGANIZATION_URL . '/reports.php', 
...
if($CFG->version<2014051200){
	add_to_log($course->id, MOD_ORGANIZATION_MODNAME, 'reports', "reports.php?id={$cm->id}", $moduleinstance->name, $cm->id);
}else{
	...
	$event->add_record_snapshot(MOD_ORGANIZATION_MODNAME, $moduleinstance);
	$event->trigger();
} 
...
$renderer = $PAGE->get_renderer(MOD_ORGANIZATION_FRANKY);
$reportrenderer = $PAGE->get_renderer(MOD_ORGANIZATION_FRANKY,'report');
...
switch ($showreport){
	...
	case 'menu':
		echo $renderer->header($moduleinstance, $cm, $mode, null, get_string('reports', MOD_ORGANIZATION_LANG));
	...
	case 'basic':
		$report = new mod_organization_basic_report();
